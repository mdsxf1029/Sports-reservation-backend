...
public class NewsController(OracleDbContext context, IWebHostEnvironment env) : ControllerBase
{
    ...
    [HttpGet]
    [SwaggerOperation(Summary = "获取新闻表信息", Description = "获取新闻表信息（支持多条件筛选）")]
    public async Task<ActionResult<IEnumerable<News>>> GetNews(
        [FromQuery] int page = 1,
        [FromQuery] int pageSize = 10,
        [FromQuery] string? status = null, // 状态筛选
        [FromQuery] string? keyword = null, // 标题关键词筛选
        [FromQuery] List<string>? dateRange = null // 时间范围筛选（["2025-09-01","2025-09-10"]）
    )
    {
        if (page < 1)
            page = 1;
        ...
        try
        {
            ...
            {
                query = query.Where(n => n.NewsTitle.Contains(keyword));
            }
            ...
            {
                if (
                    DateTime.TryParse(dateRange[0], out var startDate)
                    && DateTime.TryParse(dateRange[1], out var endDate)
                )
                {
                    // 让 endDate 包含当日整天
                    endDate = endDate.Date.AddDays(1).AddTicks(-1);
                    query = query.Where(n => n.NewsTime >= startDate && n.NewsTime <= endDate);
                }
            }

            // === 统计数量 ===
            var allNewsCount = await context.NewsSet.CountAsync();
            var publishedNewsCount = await context.NewsSet.CountAsync(n =>
                n.NewsStatus == "published"
            );
            var draftNewsCount = await context.NewsSet.CountAsync(n => n.NewsStatus == "draft");
    ...
    )
    {
        ...
        {
            var query = context
                .NewsSet.Where(n => (n.NewsStatus == "published" || n.NewsStatus == "updated"))
                .AsQueryable();
